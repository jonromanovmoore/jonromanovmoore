...
d}},{filter:["items","settings"],run:function(){var a=[],b=this._items,f=this.settings,d=Math.max(2*f.items,4),h=2*Math.ceil(b.length/2);f=f.loop&&b.length?f.rewind?d:Math.max(d,h):0;h=d="";for(f/=2;0<f;)a.push(this.normalize(a.length/2,!0)),d+=b[a[a.length-1]][0].outerHTML,a.push(this.normalize(b.length-1-(a.length-1)/2,!0)),h=b[a[a.length-1]][0].outerHTML+h,--f;this._clones=a;c(d).addClass("cloned").appendTo(this.$stage);c(h).addClass("cloned").prependTo(this.$stage)}},{filter:["width","items",
...
run:function(){this.animate(this.coordinates(this._current))}},{filter:["width","position","items","settings"],run:function(){var a=this.settings.rtl?1:-1,b=2*this.settings.stagePadding,c=this.coordinates(this.current())+b,d=c+this.width()*a,h=[],g;var e=0;for(g=this._coordinates.length;e<g;e++){var k=this._coordinates[e-1]||0;var l=Math.abs(this._coordinates[e])+b*a;(this.op(k,"\x3c\x3d",c)&&this.op(k,"\x3e",d)||this.op(l,"\x3c",c)&&this.op(l,"\x3e",d))&&h.push(e)}this.$stage.children(".active").removeClass("active");
...
this.$element.is(":visible"):!0};d.prototype.setup=function(){var a=this.viewport(),b=this.options.responsive,f=-1,d=null;b?(c.each(b,function(b){b<=a&&b>f&&(f=Number(b))}),d=c.extend({},this.options,b[f]),"function"===typeof d.stagePadding&&(d.stagePadding=d.stagePadding()),delete d.responsive,d.responsiveClass&&this.$element.attr("class",this.$element.attr("class").replace(new RegExp("("+this.options.responsiveClass+"-)\\S+\\s","g"),"$1"+f))):d=c.extend({},this.options);this.trigger("change",{property:{name:"settings",
...
this.enter("resizing");if(this.trigger("resize").isDefaultPrevented())return this.leave("resizing"),!1;this.invalidate("width");this.refresh();this.leave("resizing");this.trigger("resized")};d.prototype.registerEventHandlers=function(){if(c.support.transition)this.$stage.on(c.support.transition.end+".owl.core",c.proxy(this.onTransitionEnd,this));if(!1!==this.settings.responsive)this.on(e,"resize",this._handlers.onThrottledResize);this.settings.mouseDrag&&(this.$element.addClass(this.options.dragClass),
...
var b=this._drag.stage.current,f=0<a.x^this.settings.rtl?"left":"right";c(l).off(".owl.core");this.$element.removeClass(this.options.grabClass);if(0!==a.x&&this.is("dragging")||!this.is("valid"))if(this.speed(this.settings.dragEndSpeed||this.settings.smartSpeed),this.current(this.closest(b.x,0!==a.x?f:this._drag.direction)),this.invalidate("position"),this.update(),this._drag.direction=f,3<Math.abs(a.x)||300<(new Date).getTime()-this._drag.time)this._drag.target.one("click.owl.core",function(){return!1});
this.is("dragging")&&(this.leave("dragging"),this.trigger("dragged"))};d.prototype.closest=function(a,b){var f=-1,d=this.width(),h=this.coordinates();this.settings.freeDrag||c.each(h,c.proxy(function(c,m){"left"===b&&a>m-30&&a<m+30?f=c:"right"===b&&a>m-d-30&&a<m-d+30?f=c+1:this.op(a,"\x3c",m)&&this.op(a,"\x3e",h[c+1]!==k?h[c+1]:m-d)&&(f="left"===b?c+1:c);return-1===f},this));this.settings.loop||(this.op(a,"\x3e",h[this.minimum()])?f=a=this.minimum():this.op(a,"\x3c",h[this.maximum()])&&(f=a=this.maximum()));
...
c.type(a)&&(this._invalidated[a]=!0,this.is("valid")&&this.leave("valid"));return c.map(this._invalidated,function(a,c){return c})};d.prototype.reset=function(a){a=this.normalize(a);a!==k&&(this._speed=0,this._current=a,this.suppress(["translate","translated"]),this.animate(this.coordinates(a)),this.release(["translate","translated"]))};d.prototype.normalize=function(a,b){var c=this._items.length;b=b?0:this._clones.length;if(!this.isNumeric(a)||1>c)a=k;else if(0>a||a>=c+b)a=((a-b/2)%c+c)%c+b/2;return a};
...
typeof this.settings[h]&&this.settings[h].call(this,g));return g};d.prototype.enter=function(a){c.each([a].concat(this._states.tags[a]||[]),c.proxy(function(a,c){this._states.current[c]===k&&(this._states.current[c]=0);this._states.current[c]++},this))};d.prototype.leave=function(a){c.each([a].concat(this._states.tags[a]||[]),c.proxy(function(a,c){this._states.current[c]--},this))};d.prototype.register=function(a){if(a.type===d.Type.Event){if(c.event.special[a.name]||(c.event.special[a.name]={}),
...
function(a){return!isNaN(parseFloat(a))};d.prototype.difference=function(a,b){return{x:a.x-b.x,y:a.y-b.y}};c.fn.owlCarousel=function(a){var b=Array.prototype.slice.call(arguments,1);return this.each(function(){var f=c(this),m=f.data("owl.carousel");m||(m=new d(this,"object"==typeof a&&a),f.data("owl.carousel",m),c.each("next prev to destroy refresh replace add remove".split(" "),function(a,b){m.register({type:d.Type.Event,name:b});m.$element.on(b+".owl.carousel.core",c.proxy(function(a){a.namespace&&
a.relatedTarget!==this&&(this.suppress([b]),m[b].apply(this,[].slice.call(arguments,1)),this.release([b]))},m))}));"string"==typeof a&&"_"!==a.charAt(0)&&m[a].apply(m,b)})};c.fn.owlCarousel.Constructor=d})(window.Zepto||window.jQuery,window,document);
(function(c,e,l,k){var d=function(a){this._core=a;this._visible=this._interval=null;this._handlers={"initialized.owl.carousel":c.proxy(function(a){a.namespace&&this._core.settings.autoRefresh&&this.watch()},this)};this._core.options=c.extend({},d.Defaults,this._core.options);this._core.$element.on(this._handlers)};d.Defaults={autoRefresh:!0,autoRefreshInterval:500};d.prototype.watch=function(){this._interval||(this._visible=this._core.isVisible(),this._interval=e.setInterval(c.proxy(this.refresh,
this),this._core.settings.autoRefreshInterval))};d.prototype.refresh=function(){this._core.isVisible()!==this._visible&&(this._visible=!this._visible,this._core.$element.toggleClass("owl-hidden",!this._visible),this._visible&&this._core.invalidate("width")&&this._core.refresh())};d.prototype.destroy=function(){var a,b;e.clearInterval(this._interval);for(a in this._handlers)this._core.$element.off(a,this._handlers[a]);for(b in Object.getOwnPropertyNames(this))"function"!=typeof this[b]&&(this[b]=null)};
c.fn.owlCarousel.Constructor.Plugins.AutoRefresh=d})(window.Zepto||window.jQuery,window,document);
(function(c,e,l,k){var d=function(a){this._core=a;this._loaded=[];this._handlers={"initialized.owl.carousel change.owl.carousel resized.owl.carousel":c.proxy(function(a){if(a.namespace&&this._core.settings&&this._core.settings.lazyLoad&&(a.property&&"position"==a.property.name||"initialized"==a.type)){var b=this._core.settings,d=b.center&&Math.ceil(b.items/2)||b.items,h=b.center&&-1*d||0;a=(a.property&&a.property.value!==k?a.property.value:this._core.current())+h;var g=this._core.clones().length,
e=c.proxy(function(a,b){this.load(b)},this);0<b.lazyLoadEager&&(d+=b.lazyLoadEager,b.loop&&(a-=b.lazyLoadEager,d++));for(;h++<d;)this.load(g/2+this._core.relative(a)),g&&c.each(this._core.clones(this._core.relative(a)),e),a++}},this)};this._core.options=c.extend({},d.Defaults,this._core.options);this._core.$element.on(this._handlers)};d.Defaults={lazyLoad:!1,lazyLoadEager:0};d.prototype.load=function(a){var b=(a=this._core.$stage.children().eq(a))&&a.find(".owl-lazy");!b||-1<c.inArray(a.get(0),this._loaded)||
(b.each(c.proxy(function(a,b){var d=c(b),f=1<e.devicePixelRatio&&d.attr("data-src-retina")||d.attr("data-src")||d.attr("data-srcset");this._core.trigger("load",{element:d,url:f},"lazy");d.is("img")?d.one("load.owl.lazy",c.proxy(function(){d.css("opacity",1);this._core.trigger("loaded",{element:d,url:f},"lazy")},this)).attr("src",f):d.is("source")?d.one("load.owl.lazy",c.proxy(function(){this._core.trigger("loaded",{element:d,url:f},"lazy")},this)).attr("srcset",f):(a=new Image,a.onload=c.proxy(function(){d.css({"background-image":'url("'+
f+'")',opacity:"1"});this._core.trigger("loaded",{element:d,url:f},"lazy")},this),a.src=f)},this)),this._loaded.push(a.get(0)))};d.prototype.destroy=function(){var a,b;for(a in this.handlers)this._core.$element.off(a,this.handlers[a]);for(b in Object.getOwnPropertyNames(this))"function"!=typeof this[b]&&(this[b]=null)};c.fn.owlCarousel.Constructor.Plugins.Lazy=d})(window.Zepto||window.jQuery,window,document);
(function(c,e,l,k){var d=function(a){this._core=a;this._previousHeight=null;this._handlers={"initialized.owl.carousel refreshed.owl.carousel":c.proxy(function(a){a.namespace&&this._core.settings.autoHeight&&this.update()},this),"changed.owl.carousel":c.proxy(function(a){a.namespace&&this._core.settings.autoHeight&&"position"===a.property.name&&this.update()},this),"loaded.owl.lazy":c.proxy(function(a){a.namespace&&this._core.settings.autoHeight&&a.element.closest("."+this._core.settings.itemClass).index()===
this._core.current()&&this.update()},this)};this._core.options=c.extend({},d.Defaults,this._core.options);this._core.$element.on(this._handlers);this._intervalId=null;var b=this;c(e).on("load",function(){b._core.settings.autoHeight&&b.update()});c(e).resize(function(){b._core.settings.autoHeight&&(null!=b._intervalId&&clearTimeout(b._intervalId),b._intervalId=setTimeout(function(){b.update()},250))})};d.Defaults={autoHeight:!1,autoHeightClass:"owl-height"};d.prototype.update=function(){var a=this._core._current,
b=a+this._core.settings.items,d=this._core.settings.lazyLoad;a=this._core.$stage.children().toArray().slice(a,b);var e=[];b=0;c.each(a,function(a,b){e.push(c(b).height())});b=Math.max.apply(null,e);1>=b&&d&&this._previousHeight&&(b=this._previousHeight);this._previousHeight=b;this._core.$stage.parent().height(b).addClass(this._core.settings.autoHeightClass)};d.prototype.destroy=function(){var a,b;for(a in this._handlers)this._core.$element.off(a,this._handlers[a]);for(b in Object.getOwnPropertyNames(this))"function"!==
typeof this[b]&&(this[b]=null)};c.fn.owlCarousel.Constructor.Plugins.AutoHeight=d})(window.Zepto||window.jQuery,window,document);
(function(c,e,l,k){var d=function(a){this._core=a;this._videos={};this._playing=null;this._handlers={"initialized.owl.carousel":c.proxy(function(a){a.namespace&&this._core.register({type:"state",name:"playing",tags:["interacting"]})},this),"resize.owl.carousel":c.proxy(function(a){a.namespace&&this._core.settings.video&&this.isInFullScreen()&&a.preventDefault()},this),"refreshed.owl.carousel":c.proxy(function(a){a.namespace&&this._core.is("resizing")&&this._core.$stage.find(".cloned .owl-video-frame").remove()},
...
l.lazyLoad?c("\x3cdiv/\x3e",{"class":"owl-video-tn "+k,srcType:b}):c("\x3cdiv/\x3e",{"class":"owl-video-tn",style:"opacity:1;background-image:url("+b+")"});a.after(d);a.after('\x3cdiv class\x3d"owl-video-play-icon"\x3e\x3c/div\x3e')};a.wrap(c("\x3cdiv/\x3e",{"class":"owl-video-wrapper",style:e}));this._core.settings.lazyLoad&&(g="data-src",k="owl-lazy");if(h.length)return p(h.attr(g)),h.remove(),!1;if("youtube"===b.type){var n="//img.youtube.com/vi/"+b.id+"/hqdefault.jpg";p(n)}else"vimeo"===b.type?
...
this._core.trigger("stopped",null,"video")};d.prototype.play=function(a){a=c(a.target).closest("."+this._core.settings.itemClass);var b=this._videos[a.attr("data-video")],d=b.width||"100%",e=b.height||this._core.$stage.height();if(!this._playing){this._core.enter("playing");this._core.trigger("play",null,"video");a=this._core.items(this._core.relative(a.index()));this._core.reset(a.index());var h=c('\x3ciframe frameborder\x3d"0" allowfullscreen mozallowfullscreen webkitAllowFullScreen \x3e\x3c/iframe\x3e');
h.attr("height",e);h.attr("width",d);"youtube"===b.type?h.attr("src","//www.youtube.com/embed/"+b.id+"?autoplay\x3d1\x26rel\x3d0\x26v\x3d"+b.id):"vimeo"===b.type?h.attr("src","//player.vimeo.com/video/"+b.id+"?autoplay\x3d1"):"vzaar"===b.type&&h.attr("src","//view.vzaar.com/"+b.id+"/player?autoplay\x3dtrue");c(h).wrap('\x3cdiv class\x3d"owl-video-frame" /\x3e').insertAfter(a.find(".owl-video"));this._playing=a.addClass("owl-video-playing")}};d.prototype.isInFullScreen=function(){var a=l.fullscreenElement||
l.mozFullScreenElement||l.webkitFullscreenElement;return a&&c(a).parent().hasClass("owl-video-frame")};d.prototype.destroy=function(){var a,b;this._core.$element.off("click.owl.video");for(a in this._handlers)this._core.$element.off(a,this._handlers[a]);for(b in Object.getOwnPropertyNames(this))"function"!=typeof this[b]&&(this[b]=null)};c.fn.owlCarousel.Constructor.Plugins.Video=d})(window.Zepto||window.jQuery,window,document);
(function(c,e,l,k){var d=function(a){this.core=a;this.core.options=c.extend({},d.Defaults,this.core.options);this.swapping=!0;this.next=this.previous=k;this.handlers={"change.owl.carousel":c.proxy(function(a){a.namespace&&"position"==a.property.name&&(this.previous=this.core.current(),this.next=a.property.value)},this),"drag.owl.carousel dragged.owl.carousel translated.owl.carousel":c.proxy(function(a){a.namespace&&(this.swapping="translated"==a.type)},this),"translate.owl.carousel":c.proxy(function(a){a.namespace&&
...
(function(c,e,l,k){var d=function(a){this._core=a;this._call=null;this._timeout=this._time=0;this._paused=!0;this._handlers={"changed.owl.carousel":c.proxy(function(a){a.namespace&&"settings"===a.property.name?this._core.settings.autoplay?this.play():this.stop():a.namespace&&"position"===a.property.name&&this._paused&&(this._time=0)},this),"initialized.owl.carousel":c.proxy(function(a){a.namespace&&this._core.settings.autoplay&&this.play()},this),"play.owl.autoplay":c.proxy(function(a,c,d){a.namespace&&
this.play(c,d)},this),"stop.owl.autoplay":c.proxy(function(a){a.namespace&&this.stop()},this),"mouseover.owl.autoplay":c.proxy(function(){this._core.settings.autoplayHoverPause&&this._core.is("rotating")&&this.pause()},this),"mouseleave.owl.autoplay":c.proxy(function(){this._core.settings.autoplayHoverPause&&this._core.is("rotating")&&this.play()},this),"touchstart.owl.core":c.proxy(function(){this._core.settings.autoplayHoverPause&&this._core.is("rotating")&&this.pause()},this),"touchend.owl.core":c.proxy(function(){this._core.settings.autoplayHoverPause&&
this.play()},this)};this._core.$element.on(this._handlers);this._core.options=c.extend({},d.Defaults,this._core.options)};d.Defaults={autoplay:!1,autoplayTimeout:5E3,autoplayHoverPause:!1,autoplaySpeed:!1};d.prototype._next=function(a){this._call=e.setTimeout(c.proxy(this._next,this,a),this._timeout*(Math.round(this.read()/this._timeout)+1)-this.read());this._core.is("interacting")||l.hidden||this._core.next(a||this._core.settings.autoplaySpeed)};d.prototype.read=function(){return(new Date).getTime()-
this._time};d.prototype.play=function(a,b){this._core.is("rotating")||this._core.enter("rotating");a=a||this._core.settings.autoplayTimeout;var d=Math.min(this._time%(this._timeout||a),a);this._paused?(this._time=this.read(),this._paused=!1):e.clearTimeout(this._call);this._time+=this.read()%a-d;this._timeout=a;this._call=e.setTimeout(c.proxy(this._next,this,b),a-d)};d.prototype.stop=function(){this._core.is("rotating")&&(this._time=0,this._paused=!0,e.clearTimeout(this._call),this._core.leave("rotating"))};
d.prototype.pause=function(){this._core.is("rotating")&&!this._paused&&(this._time=this.read(),this._paused=!0,e.clearTimeout(this._call))};d.prototype.destroy=function(){var a,b;this.stop();for(a in this._handlers)this._core.$element.off(a,this._handlers[a]);for(b in Object.getOwnPropertyNames(this))"function"!=typeof this[b]&&(this[b]=null)};c.fn.owlCarousel.Constructor.Plugins.autoplay=d})(window.Zepto||window.jQuery,window,document);
(function(c,e,l,k){var d=function(a){this._core=a;this._initialized=!1;this._pages=[];this._controls={};this._templates=[];this.$element=this._core.$element;this._overrides={next:this._core.next,prev:this._core.prev,to:this._core.to};this._handlers={"prepared.owl.carousel":c.proxy(function(a){a.namespace&&this._core.settings.dotsData&&this._templates.push('\x3cdiv class\x3d"'+this._core.settings.dotClass+'"\x3e'+c(a.content).find("[data-dot]").addBack("[data-dot]").attr("data-dot")+"\x3c/div\x3e")},
...
this._controls.$absolute=(b.dotsContainer?c(b.dotsContainer):c("\x3cdiv\x3e").addClass(b.dotsClass).appendTo(this.$element)).addClass("disabled");this._controls.$absolute.on("click","button",c.proxy(function(a){var d=c(a.target).parent().is(this._controls.$absolute)?c(a.target).index():c(a.target).parent().index();a.preventDefault();this.to(d,b.dotsSpeed)},this));for(a in this._overrides)this._core[a]=c.proxy(this[a],this)};d.prototype.destroy=function(){var a,b,c,d;var e=this._core.settings;for(a in this._handlers)this.$element.off(a,
this._handlers[a]);for(b in this._controls)"$relative"===b&&e.navContainer?this._controls[b].html(""):this._controls[b].remove();for(d in this.overides)this._core[d]=this._overrides[d];for(c in Object.getOwnPropertyNames(this))"function"!=typeof this[c]&&(this[c]=null)};d.prototype.update=function(){var a,b,c=this._core.clones().length/2,d=c+this._core.items().length,e=this._core.maximum(!0);var g=this._core.settings;var k=g.center||g.autoWidth||g.dotsData?1:g.dotsEach||g.items;"page"!==g.slideBy&&
(g.slideBy=Math.min(g.slideBy,g.items));if(g.dots||"page"==g.slideBy)for(this._pages=[],g=c,b=a=0;g<d;g++){if(a>=k||0===a){this._pages.push({start:Math.min(e,g-c),end:g-c+k-1});if(Math.min(e,g-c)===e)break;a=0;++b}a+=this._core.mergers(this._core.relative(g))}};d.prototype.draw=function(){var a=this._core.settings;var b=this._core.items().length<=a.items;var d=this._core.relative(this._core.current()),e=a.loop||a.rewind;this._controls.$relative.toggleClass("disabled",!a.nav||b);a.nav&&(this._controls.$previous.toggleClass("disabled",
...
(function(c,e,l,k){function d(d,e){var f=!1,h=d.charAt(0).toUpperCase()+d.slice(1);c.each((d+" "+b.join(h+" ")+h).split(" "),function(b,c){if(a[c]!==k)return f=e?c:!0,!1});return f}var a=c("\x3csupport\x3e").get(0).style,b=["Webkit","Moz","O","ms"];e={WebkitTransition:"webkitTransitionEnd",MozTransition:"transitionend",OTransition:"oTransitionEnd",transition:"transitionend"};l={WebkitAnimation:"webkitAnimationEnd",MozAnimation:"animationend",OAnimation:"oAnimationEnd",animation:"animationend"};var f=
{csstransforms:function(){return!!d("transform")},csstransforms3d:function(){return!!d("perspective")},csstransitions:function(){return!!d("transition")},cssanimations:function(){return!!d("animation")}};f.csstransitions()&&(c.support.transition=new String(d("transition",!0)),c.support.transition.end=e[c.support.transition]);f.cssanimations()&&(c.support.animation=new String(d("animation",!0)),c.support.animation.end=l[c.support.animation]);f.csstransforms()&&(c.support.transform=new String(d("transform",
!0)),c.support.transform3d=f.csstransforms3d())})(window.Zepto||window.jQuery,window,document);
$(document).ready(function(){if(0<$(".bannerSlider").length){var c=$(".bannerSlider").owlCarousel({pullDrag:!1,nav:!1,dots:!0,navText:["",""],items:1,autoHeight:!0,autoplay:!0,autoplayTimeout:5E3,loop:!0,mouseDrag:!1});c.on("initialized.owl.carousel",function(c){$(".owl-item:not(.active)").attr("tabindex","-1").attr("aria-selected","false").find("a").attr("tabindex","-1");$(".owl-item.active").attr("aria-selected","true").find("a").attr("tabindex","0")});c.on("translated.owl.carousel",function(c){$(".owl-item:not(.active)").attr("tabindex",
"-1").attr("aria-selected","false").find("a").attr("tabindex","-1");$(".owl-item.active").attr("aria-selected","true").find("a").attr("tabindex","0")});if(!$(".bannerSlider .owl-dots").hasClass("disabled")){var e=$(".bannerSlider .owl-dots").position().left+$(".bannerSlider .owl-dots").outerWidth();$(".bannerSlider").append('\x3cbutton class\x3d"play-button" style\x3d"left:'+e+'px"\x3e\x3cspan class\x3d"fa fa-pause"\x3e\x3c/span\x3e\x3c/button\x3e')}$(document).on("click",".play-button",function(){$(this).toggleClass("active");
$(this).hasClass("active")?c.trigger("stop.owl.autoplay"):c.trigger("play.owl.autoplay",[5E3])});bannerFontUpdate()}});function bannerFontUpdate(){1024<$(window).width()?$(".bannerSlider-caption .bannerTitle[data-deskfontSize]").each(function(){$(this).find("h1").css("font-size",$(this).attr("data-deskfontSize")+"px")}):$(".bannerSlider-caption .bannerTitle[data-mobfontsize]").each(function(){$(this).find("h1").css("font-size",$(this).attr("data-mobfontsize")+"px")})}
$(window).resize(function(){$(".play-button").length&&($(".play-button").removeClass("active"),$(".play-button").css("left",$(".bannerSlider .owl-dots").position().left+$(".bannerSlider .owl-dots").outerWidth()));bannerFontUpdate()});